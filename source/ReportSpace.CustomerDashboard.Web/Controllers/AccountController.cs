...
namespace ReportSpace.CustomerDashboard.Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            if (_manager.UserExists(model.UserName))
            {
                var auth_result = _manager.ValidateDataBaseUser(model.UserName, model.Password);
                if (auth_result)
                {
                    // We are good! 
                    bool persist_cookie = false;
                    if(model.RememberMe != null) 
                    {
                        persist_cookie = model.RememberMe;
                    }

                    bool login_result = WebSecurity.Login(model.UserName, model.Password, persist_cookie);
                    if (login_result)
                    {
